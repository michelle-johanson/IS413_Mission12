...
**Bootstrap Comment**
1. Positioned badge (`position-absolute top-0 start-100 translate-middle badge rounded-pill`) — the red item-count bubble on the Cart button in the header
2. `rounded-pill` badge — used to display the active category filter as a dismissible pill badge with an inline close button
